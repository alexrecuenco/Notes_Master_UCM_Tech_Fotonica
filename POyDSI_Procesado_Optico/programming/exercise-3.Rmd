...
Where $\phi = e^{i \frac{\pi}{3}}$ and $k = \frac 1 {2\abs{a}}\textrm{tri} \left(\frac{u_0}{a}\right)$


Note that I am not confident this is correct, because we are getting some imaginary part in the result. But I can't correct this within the time I have left. I hope to do that later.

The maximum contrast would be achieved when the difference of the minimum and maximum on the obtained function is maximal. This is entirely modulated by $k$.

Maximizing $k$ implies maximizing the following, assuming $a$ is bigger tha $u_0$
...
\begin{equation*}
\frac 1 {2a^2} -2\frac{u_0}{a^3} = 0
\implies
\frac 1 {2} -2\frac{u_0}{a} = 0
\implies
a=4 u_0
\end{equation*}

Therefore $a = 4 u_0$ seems to be the right number. (Assuming it is indeed a maxima.) This is conveniently below the $5u_0$ limit that would bring the noise we were trying to remove.
...
f <- function(x, phi = exp(1i*pi/3)) {
	  phi * exp(2i * pi * x) + 1/phi * exp(-2i*pi*x)
}
y <- f(x)
...
I haven't had time to solve it yet. My bad. ImageJ seems to be acting up on my machine
